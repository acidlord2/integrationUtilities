<?php
	...
	$ordersBeru = OrdersBeru::getOrders (BERU_API_MARKET4CLEANING_CAMPAIGN, $filters);
	$logger->write ('01-ordersBeru - ' . json_encode ($ordersBeru, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE));
	...
	foreach ($ordersBeru as $orderBeru)
	{
		...
		//OrdersBeru::packOrder (BERU_API_MARKET4CLEANING_CAMPAIGN, $orderBeru['id'], $orderBeru['delivery']['shipments'][0]['id'], $boxes);
		
		$statusData = array (
			'order' => array (
				'status' => $orderBeru['delivery']['type'] //'DELIVERY',
				//'substatus' => 'READY_TO_SHIP'
			)
		);
		OrdersBeru::updateOrderStatus (BERU_API_MARKET4CLEANING_CAMPAIGN, $orderBeru['id'], $statusData);
		$orderLabels = OrdersBeru::getOrdersLebels (BERU_API_MARKET4CLEANING_CAMPAIGN, $orderBeru['id']);
