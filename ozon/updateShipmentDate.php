<?php
	...
	$logger = new Log ('updateShipmentDate.log');
	//$newOrdersOzon = Orders::getOzonOrders('2019-12-16T10:57:21Z', '2020-12-16T11:57:21Z', "awaiting_packaging");
	date_default_timezone_set('Europe/Moscow');
	$ordersOzon = Orders::getOzonOrders(date ('Y-m-d', strtotime('-5 days')) . 'T00:00:00Z', date ('Y-m-d', strtotime('-2 days')) . 'T23:59:59Z', "awaiting_deliver");
	$logger -> write ('ordersOzon: ' . json_encode($ordersOzon, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE));
	if (count ($ordersOzon) > 0)
		foreach ($ordersOzon as $order)
		{
			$ms_order = Orders::findOrder ($order['posting_number']);
			$logger -> write ('ms_order: ' . json_encode($ms_order, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE));
