<?php
	...
	*/
	...
	// check auth-token
	if (isset($_GET['auth-token']) ? (string)$_GET['auth-token'] != Settings::getSettingsValues('abcd-auth-token') : true)
	{
		...
		if ($idKey !== false)
		{
			...
			$skus = array (
				...
				'items' => array (
					0 => array (
						'type' => 'FIT',
						'count' => $beruPriceKey !== false ? ((float)$product['salePrices'][$beruPriceKey]['value'] / 100 > 800 ? (int)$product['quantity'] : 0) : 0,
