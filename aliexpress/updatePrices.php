<?php
	...
	$logger = new Log ('aliexpress - updatePrices.log');
	...
	foreach ($products as $key => $product)
	{
		...
		if ($idKey === false)
		{
		    $logger->write (__LINE__ . ' Не найден код товара в МС - ' . $product['sku']);
			continue;
		}
		$priceKey = array_search ('Цена ALI', array_column ($stock[$idKey]['salePrices'], 'priceType'));
		if ($priceKey === false)
		{
		    $logger->write (__LINE__ . ' Для товара не задана цена ALI в МС - ' . $product['sku']);
			continue;
		}
		$i++;
		...
		if ($i == 20 || $key + 1 == count ($products))
		{
			...
			$logger->write (__LINE__ . ' params - ' . json_encode ($params, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE));
