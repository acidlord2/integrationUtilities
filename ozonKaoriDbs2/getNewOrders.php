...
//	require_once($_SERVER['DOCUMENT_ROOT'] . '/classes/orders.php');
	...
	$log = new Log ('ozonKaoriDbs2 - getNewOrders.log');
	$ordersOzonClass = new OrdersOzon('kaori');
	
	$ordersOzon = $ordersOzonClass->findOrders(date ('Y-m-d', strtotime('-1 day')) . 'T00:00:00Z', date ('Y-m-d', strtotime('now')) . 'T23:59:59Z', 'awaiting_packaging', OZON_WEARHOUSE3_ID);
	...
	foreach ($ordersOzon as $order)
	{
		...
		$data['organization'] = array(
		    'meta' => APIMS::createMeta (MS_API_BASE_URL. MS_API_VERSION_1_2. MS_API_ORGANIZATION . '/' . MS_KAORI_ID, 'organization')
		);
		...
		$data['project'] = array(
		    'meta' => APIMS::createMeta (MS_API_BASE_URL. MS_API_VERSION_1_2. MS_API_PROJECT . '/' . MS_PROJECT_OZON_DBS_ID, 'project')
		);
		$data['group'] = array(
