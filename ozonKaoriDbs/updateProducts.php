<?php
	...
	$productOzonClass = new ProductsOzon('kaori');
	$log = new Log ('ozonKaoriDbs - UpdateProducts.log');
	...
	foreach ($productsMS as $key => $productMS)
	{
		...
		array_push ($prices['prices'], array ('offer_id' => $productMS['code'], 'price' => (string)$price, 'old_price' => '0'));
		array_push ($stocks['stocks'], array ('offer_id' => $productMS['code'], 'stock' => ($price == 0 ? 0 : $quantity), 'warehouse_id' => OZON_WEARHOUSE2_ID));
		//array_push ($stocks['stocks'], array ('offer_id' => $productMS['code'], 'stock' => 0, 'warehouse_id' => OZON_WEARHOUSE2_ID));
